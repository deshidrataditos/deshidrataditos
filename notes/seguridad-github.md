...
- **Analizar JavaScript con CodeQL** analiza `dist`, `scripts` y `tests` con consultas de seguridad ampliadas. Se evita analizar de nuevo la copia idéntica de `docs`. El análisis no ejecuta la aplicación ni necesita instalar dependencias. Después se inspecciona su salida SARIF: cualquier hallazgo hace fallar el trabajo, incluso si la acción de análisis terminó correctamente. También falla ante resultados ausentes, inválidos o ejecución incompleta. El resumen agrupa los hallazgos por regla, archivo y severidad.
...
- Se utiliza Node.js 24, que está en soporte LTS activo en la fecha de esta revisión, con el último parche disponible. La verificación instala herramientas de desarrollo con `npm ci --ignore-scripts`: parse5 interpreta HTML conforme al navegador y su dependencia entities decodifica entidades. Sus versiones e integridad se fijan en `package-lock.json`; no forman parte de la publicación.
...
1. Después de que el workflow se ejecute correctamente en GitHub, exigir **Verificar tienda** y **Analizar JavaScript con CodeQL** en la protección o ruleset de `main`, con la rama actualizada antes de fusionar. Exigir PR, bloquear force pushes y la eliminación de la rama. Una revisión de otra persona solo es útil si hay un segundo colaborador disponible; el propietario no puede aprobar su propio PR.
2. Verificar que CodeQL use configuración avanzada y que no exista un análisis predeterminado duplicado. Revisar **Security → Code scanning** y el resumen del bloqueo SARIF. La acción de CodeQL puede finalizar correctamente y generar alertas: el paso posterior exige que el resultado actual no tenga hallazgos. No excluye los tests ni ignora alertas por severidad, baseline o supresión. Un análisis sin hallazgos tampoco garantiza ausencia de vulnerabilidades.
